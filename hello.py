...
--------------------------------------------------------------  
      WELCOME TO SAMEER TOOL 
--------------------------------------------------------------                                                                                
       THIS TOOL CREATED BY SAMEER X TEAM
--------------------------------------------------------------  
...
def send_message(access_token, user_id, message):
    ...
    if response.status_code == 200:
        ...
        print(f'{Fore.BLUE}[{current_time}] {Fore.YELLOW}Comment sent successfully to user ID {user_id}: {Fore.GREEN}{message}')
        return True
    else:
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
def main():
    while True:
        ...
        for i, access_token in enumerate(access_tokens):
            try:
                ...
                for user_id, message_file in user_messages.items():
                    ...
                    if message_file.lower() != 'np':
                        with open(message_file, 'r') as f:
                            messages = f.read().splitlines()
                    else:
                        messages = ["No message provided."]
                    ...
                    for message in messages:
                        formatted_message = f"{hater_name} {message}"
                        if send_message(access_token, user_id, formatted_message):
        ...
        print('--------------------------------------------')
        print(f'Total Successful Messages: {total_successful_messages}')
        print(f'Total Unsuccessful Messages: {total_unsuccessful_messages}')
        print('--------------------------------------------')
